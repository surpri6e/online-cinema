    ...
    <div className="fpw">
      <div className="_Container">
        <div className="fpw_body">
          ...
          <FilmsPageBlock title="Dramas" items={dataFirst?.concat(dataSecond!).concat(dataThird!).filter(item => item.genres[0].genre === 'драма') as IFilmSmall[]} nextElementClass="next_element_cartoons_dramas"/>
          <FilmsPageBlock title="Adventures" items={dataFirst?.concat(dataSecond!).concat(dataThird!).filter(item => item.genres[0].genre === 'приключения') as IFilmSmall[]} nextElementClass="next_element_cartoons_adventures"/>
          <FilmsPageBlock title="Melodramas" items={dataFirst?.concat(dataSecond!).concat(dataThird!).filter(item => item.genres[0].genre === 'мелодрама') as IFilmSmall[]} nextElementClass="next_element_cartoons_melodramas"/>
          <Thanks/>
        </div>
      </div>
    </div>
